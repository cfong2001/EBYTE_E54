...
## 2026-06-15 - Dynamic Centering using snprintf for Diagnostic Text
**Learning:** For screens that print text combined with logic output (e.g., test PASS/FAIL cases, or formatted testing dots `...`), calculating string length natively using `textWidth` to dynamically adjust the layout is necessary to avoid odd unaligned layouts when logic output varies in length.
**Action:** Use string variables built by string formatting via `snprintf` or conditional inline blocks to evaluate logic values and dynamically set the `X` position (like `sprite.setCursor((240 - tw) / 2, yPos)`) instead of hardcoded coordinates.
