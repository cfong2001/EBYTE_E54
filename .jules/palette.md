...
## 2026-06-18 - Centering text with Adafruit_GFX bounds
**Learning:** When using OLED displays powered by `Adafruit_SSD1306` or `Adafruit_GFX`, the library does not provide a direct `textWidth()` helper method like `TFT_eSPI` does.
**Action:** When centering text dynamically on these displays, use `getTextBounds(string, 0, 0, &x1, &y1, &w, &h)` to extract the width (`w`) for centering calculations (e.g., `(screenWidth - w) / 2`).
