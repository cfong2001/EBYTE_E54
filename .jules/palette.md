...
## 2024-06-29 - Dynamic Layout for Multi-Line Screens
**Learning:** Hardcoding Y-coordinates on multi-line text screens like self-test results in visual overlapping and poor accessibility when users increase UI text size settings.
**Action:** When creating text layouts that handle conditional block rendering, replace fixed numbers with dynamic scaling multipliers (e.g., `lineSpace = 8 * uiTextSize + 4`) and accumulate offsets consistently across all conditional branches to avoid jittering text blocks during state changes.
