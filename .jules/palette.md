...
## 2026-06-24 - Responsive Multi-line Text Rendering
**Learning:** Hardcoding Y-coordinates (e.g., `sprite.setCursor(10, 40)`) for consecutive lines in multi-line text displays causes text to overlap and become illegible when users increase accessibility settings like `uiTextSize`.
**Action:** Always compute Y offsets dynamically based on the current font scale (e.g., `y + (8 * uiTextSize)`) to ensure text blocks scale proportionally and maintain proper line height when accessibility settings change.
