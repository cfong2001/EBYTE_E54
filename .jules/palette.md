...
## 2026-06-15 - Dynamic Conditional Multi-Line Text Y Offsets
**Learning:** Hardcoded Y coordinates in multi-line layouts break accessibility, especially when text scales change. Even if the Y coordinates are computed relative to `uiTextSize`, a failure to advance the `y` variable correctly across conditional branching paths will cause subsequent UI elements to jitter vertically between states.
**Action:** When calculating relative conditional vertical offsets (e.g., advancing `y` inside branching `if/else` states), strictly ensure `y` is advanced properly within both branches so the next component remains vertically anchored regardless of which state is rendered.
