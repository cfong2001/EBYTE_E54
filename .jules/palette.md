...
## 2024-06-26 - Dynamic Text Scaling Offsets for Multi-line Screens
**Learning:** Hardcoding vertical Y-coordinates (like `160`, `80`, `100`) for multi-line screens and tooltips causes severe text overlapping and visual jitter when accessibility settings like text scaling (`uiTextSize`) are increased.
**Action:** When implementing multi-line text screens or tooltips, always calculate dynamic Y-offsets based on the current font scale (e.g., `y + (8 * uiTextSize) + padding`). Furthermore, ensure the vertical offset variable is consistently incremented across all conditional rendering branches (like loading states vs. completed results) so subsequent elements do not jitter horizontally or vertically during state transitions.
