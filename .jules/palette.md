...
## 2026-05-24 - [Exposing Backend Generated Secrets in UI]
**Learning:** If a background service (like a WiFi Access Point) dynamically generates a critical piece of information (like a password), it is entirely opaque to the user unless explicitly surfaced in the UI.
**Action:** Always provide a mechanism within the hardware menu (e.g., adding an "AP PWD: <pwd>" row) to allow users to verify or retrieve dynamically generated secrets necessary for interacting with the device. Ensure this UI component dynamically adapts its visibility based on whether the feature (like the AP) is currently active to prevent confusion.
