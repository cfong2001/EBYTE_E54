...
## 2026-06-14 - Responsive Multi-line Background Rectangles and Text Positioning
**Learning:** Hardcoding Y-coordinates or bounding box heights (e.g., `fillRect(x, 40, w, 40)`) in multi-line test screens causes text lines to overlap and overflow their containers when users increase the accessibility text scaling (`uiTextSize`).
**Action:** Always compute a dynamic line height (`lh = 8 * uiTextSize + padding`) and apply relative offsets (e.g., `baseY + lh * n`) for all vertically stacked multi-line text elements. Furthermore, ensure any surrounding background rectangles or bounding boxes use dynamically calculated heights based on this line height to properly contain the text across all accessibility settings.
