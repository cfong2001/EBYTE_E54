...
## 2026-06-15 - Dynamic yOffset in Multi-line Transient Screens
**Learning:** Using hardcoded Y coordinates (e.g. `60`, `80`, `100`) for text layout in transient multi-line screens (like a Self Test screen) causes overlap and layout breakage when text scaling accessibility settings (`uiTextSize`) change.
**Action:** Always implement a dynamic vertical cursor position variable (e.g., `yOffset`) that increments relative to the active text size (`yOffset += (8 * uiTextSize) + spacing`) to ensure proportional scaling. Additionally, ensure the offset is consistently updated across all logical branches (such as 'loading' vs 'completed' states) to prevent subsequent UI elements from shifting or overlapping unexpectedly.
