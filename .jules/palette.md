...
## 2026-06-01 - Dynamic Text Centering
**Learning:** Hardcoding coordinates for text strings in transient UI states (like boot screens) breaks visual alignment when accessibility settings like text scaling are modified.
**Action:** Use responsive measurement functions like `textWidth()` combined with boundary calculations (e.g., `(tft.width() - textWidth) / 2`) to dynamically center text, ensuring visual consistency across all UI configurations.
