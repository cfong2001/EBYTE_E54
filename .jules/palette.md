...
## 2026-06-15 - Sequential Y Coordinate Propagation for Responsive Layouts
**Learning:** Hardcoding Y coordinates in transient screens (like `drawSelfTestScreen`) creates visual overlap and broken layouts when the user modifies accessibility text scaling (`uiTextSize`). Additionally, if `y` coordinate updates are isolated within conditional rendering branches (`if (loading) {...} else {...}`), elements rendered *after* the block (like footers) will jump and jitter vertically upon state transitions.
**Action:** When calculating multi-line responsive layouts using dynamic line heights (`lh = 8 * uiTextSize + 4`), always propagate the running `y` coordinate sequentially through the function. Ensure that the total `y` offset is updated consistently across all conditional paths to prevent subsequent UI elements from jittering when the layout state changes.
