...
## 2026-06-14 - Responsive Multi-line Centering
**Learning:** Hardcoding Y-coordinates (like `sprite.setCursor(10, 40)`) for multi-line UI screens breaks vertical spacing and alignment when text scaling (`uiTextSize`) changes, causing text to overlap or float unnaturally.
**Action:** When calculating vertical layouts for multi-line UI text, use dynamic Y-coordinate offsets based on the font scale (e.g., `int lineH = 8 * uiTextSize; y += lineH + padding;`). Additionally, when using dynamic formatting strings in transient states, ensure their bounding boxes are explicitly calculated and dynamically centered against the screen width to avoid horizontal jitter.
