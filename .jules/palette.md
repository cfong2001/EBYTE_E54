...
## 2025-05-18 - Centering Multiline Transient Messages
**Learning:** Hardcoding coordinates for text strings in transient UI states (like WIFI AP KEY or WIFI REGEN) breaks visual alignment when accessibility settings like text scaling are modified.
**Action:** Always use responsive measurement functions like `textWidth()` combined with boundary calculations (e.g., `(tft.width() - textWidth) / 2`) to dynamically center text, ensuring the interface remains polished across all accessibility settings.
