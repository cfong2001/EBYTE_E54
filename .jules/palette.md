...
## 2024-08-02 - Connection Status for Decoupled Web UIs
**Learning:** Decoupled or polling-based web dashboards (such as those hosted on an ESP32) can silently fail when network disconnections occur, leaving the user with stale data.
**Action:** Always implement explicit visual connection status indicators (e.g., '● LIVE' / '● OFFLINE') tied to the async fetch state to prevent user confusion during network disconnections.
