...
## 2026-06-26 - [Dynamic Multi-line UI Alignment & Anchoring]
**Learning:** Hardcoding Y-coordinates or line-heights in multi-line confirmation screens creates overlapping text when users increase accessibility text scaling (`uiTextSize`). Similarly, anchoring bottom elements (like a footer prompt) to a hardcoded baseline while intermediate elements grow dynamically causes the UI to break boundaries. Combining dynamic labels and logic states via separate `sprite.print()` calls makes centering impossible.
**Action:** When implementing multi-line text screens on small displays, always use a dynamically calculated line-height (`lh = 8 * uiTextSize + padding`). Format complete strings using `snprintf` before calling `textWidth()` to enable true centering. Finally, anchor footer elements based on a dynamic maximum height offset (`baseY + lh * maxLines + padding`) to prevent vertical jitter and overflow when text scales.
