...
## 2026-06-27 - [Consistent Dynamic Text Jitter Prevention]
**Learning:** Hardcoding Y-coordinates or using inconsistent offset variables across different state branches (like loading states vs. completed results) causes vertically jumping or jittering UI elements when the state changes.
**Action:** When calculating dynamic Y-coordinate offsets for responsive multi-line layouts, always ensure the incremented vertical offset variable (e.g., `y += (lh * lines) + padding`) resolves to exactly the same value across all conditional branches so that trailing elements (like "Click to exit") stay rooted in the same position on the screen, providing a flicker-free, stable UI experience.
