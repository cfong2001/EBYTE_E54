...
## 2025-02-12 - Dynamic Text Centering in Destructive Action Confirmation
**Learning:** Hardcoding coordinates (`sprite.setCursor(10, 100)`) for dynamic or multi-line alerts, like a Wi-Fi key regeneration confirmation (`NEW PASS: [key]`), results in misaligned, off-center UI elements.
**Action:** When rendering text alerts, always calculate the text width for each component independently and dynamically center them via boundaries (e.g., `(tft.width() - textWidth) / 2`). For concatenated dynamic strings, sum the width of the components (e.g., `sprite.textWidth("NEW PASS: ") + sprite.textWidth(newPass)`) instead of instantiating new `String` objects, to preserve heap memory and ensure perfect alignment.
