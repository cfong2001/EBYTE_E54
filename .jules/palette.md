...
**Learning:** Destructive actions like Factory Reset placed inside a scrolling menu without an explicit confirmation prompt can lead to accidental data loss. Utilizing the standard "edit mode" pattern (pressing to select, turning to change, pressing to save) is inappropriate for single-shot destructive actions.
**Action:** When implementing destructive operations (like erasing configurations), never trigger the action immediately upon selection. Always route the action to a dedicated confirmation state (e.g., `STATE_CONFIRM_RESET`) with a prominent, visually distinct prompt (using `themeDanger` and pulsating animations) that explicitly requires a button press to confirm and allows turning the encoder to safely cancel.
