...
## 2024-05-18 - Centering dynamic strings in transient UI states
**Learning:** Hardcoding coordinates or using left-aligned text for multi-line transient states (like WiFi key regeneration or Self Test diagnostics) breaks visual alignment when rendering dynamic content or changing accessibility settings like text scaling.
**Action:** When rendering dynamic content like generated passwords or diagnostic statuses, concatenate the components to compute their total `textWidth()` and dynamically center them on the display (`(tft.width() - totalWidth) / 2`). For elements that animate length (like 'RUNNING TESTS...'), compute the maximum possible string length to establish a static X-coordinate, preventing visual jitter.
