...
## 2026-06-20 - Dynamic Text Centering for Transient Key Generation
**Learning:** Hardcoding coordinates (like `x=10`) for text strings showing dynamic credentials (e.g. Wi-Fi passwords) makes the text look unbalanced or misaligned on small hardware screens, especially when user text scaling is modified.
**Action:** Use responsive measurement functions like `textWidth()` combined with boundary calculations (e.g., `(240 - width) / 2`) to dynamically center text, including dynamically concatenated strings (like passwords), ensuring the interface remains polished. Also, rely on `themePrimary` rather than hardcoded white to ensure proper contrast across themes.
