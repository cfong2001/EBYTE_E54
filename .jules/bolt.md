...
## 2024-05-19 - Defer math.sqrt calls in display loops (Bug Fix)
**Learning:** When squaring the fallback value for distances (`999.0`), ensure you convert to the correct units first before squaring. The default placeholder for out-of-range displays was `999.0` meters (which is `999000` mm). Squaring `999` meters in mm^2 results in `998,001,000,000.0`. I originally used an arbitrary large number (`999,999,999.0`), which when square rooted caused the display to show `31.6m` instead of `999.0m` placeholder.
**Action:** Always verify constants and placeholder units, especially when translating logic into a squared space (like `mm^2`).
