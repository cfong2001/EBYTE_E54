...
## 2026-05-03 - [Precalculate Trigonometric Tables for UI Rendering]
**Learning:** Calling `math.sin` and `math.cos` repeatedly inside Python UI rendering loops (e.g., drawing circles/radars) causes significant CPU overhead on constrained microcontrollers.
**Action:** Precalculate trigonometric values into static module-level tuples (e.g., `COS_TABLE`, `SIN_TABLE` mapping 0-359 degrees) and access them using integer modulo arithmetic (e.g., `int(round(angle)) % 360`) to avoid repetitive floating-point math overhead.
