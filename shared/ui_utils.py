...
# ⚡ Bolt: Precalculated trigonometric tables mapping 0-359 degrees
# Access using integer modulo arithmetic (angle % 360) to avoid repetitive math.cos/sin
...
def draw_dotted_circle(display, cx, cy, r, start_angle_deg=0, end_angle_deg=360, step_deg=None, offset_deg=0, brightness=1.0):
    ...
    for angle_deg in range(start_angle_deg, end_angle_deg + 1, step_deg):
        # ⚡ Bolt: Use precalculated trig tables to avoid expensive math.cos/sin in rendering loops
        idx = int(angle_deg + offset_deg) % 360
        ...
        # We use cy + r * SIN_TABLE[idx] which matches standard Cartesian angle rotation if y goes down.
