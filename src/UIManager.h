...
public:
    ...
    void drawSelfTestScreen() {
        ...
        int stateStartY = y;
        int stateHeight = (lineH * 4) + 20;

        if (!selfTestDone) {
            float pulse = (sinf(millis() * 0.005f) + 1.0f) * 0.5f;
            uint16_t pulseColor = sprite.alphaBlend((uint8_t)(pulse * 100.0f) + 155, themeWarning, themeBg);
            sprite.fillRect(10, stateStartY, 220, stateHeight, pulseColor);
            sprite.setTextColor(themeBg, pulseColor);
            ...
            sprite.setCursor((tft.width() - tw) / 2, stateStartY + (stateHeight - lineH) / 2);
            sprite.print(buf);
        } else {
            sprite.setCursor(10, y);
