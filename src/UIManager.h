...
public:
    ...
    void drawSelfTestScreen() {
        ...
        if (!selfTestDone) {
            ...
            int runW = sprite.textWidth(buf);
            sprite.setCursor((tft.width() - runW) / 2, 55);
            sprite.print(buf);
        } else {
            int wrRxW = sprite.textWidth("Wiring / RX Check:");
            sprite.setCursor((tft.width() - wrRxW) / 2, 40);
            sprite.print("Wiring / RX Check:");
            if (selfTestRxOk) {
                sprite.setTextColor(themeSuccess, themeBg);
                int passW = sprite.textWidth("PASS: RX is HIGH");
                sprite.setCursor((tft.width() - passW) / 2, 60);
                sprite.print("PASS: RX is HIGH");
            } else {
                sprite.setTextColor(themeDanger, themeBg);
                int failW = sprite.textWidth("FAIL: RX is LOW");
                sprite.setCursor((tft.width() - failW) / 2, 60);
                sprite.print("FAIL: RX is LOW");
            }

            sprite.setTextColor(themeText, themeBg);
            int swLogW = sprite.textWidth("Software Logic Check:");
            sprite.setCursor((tft.width() - swLogW) / 2, 80);
            sprite.print("Software Logic Check:");
            if (selfTestSoftwareOk) {
                sprite.setTextColor(themeSuccess, themeBg);
                int passW = sprite.textWidth("PASS: Tests passed");
                sprite.setCursor((tft.width() - passW) / 2, 100);
                sprite.print("PASS: Tests passed");
            } else {
                sprite.setTextColor(themeDanger, themeBg);
                int failW = sprite.textWidth("FAIL: Tests failed");
                sprite.setCursor((tft.width() - failW) / 2, 100);
        ...
        sprite.setCursor((tft.width() - exitW) / 2, 140);
