...
inline void MainMenuView::handleMenuClick(UIManager* ui) {
    ...
    if (strncmp(selItem, "VISUAL SETTINGS", 15) == 0 || strncmp(selItem, "  [DISPLAY/HUD]", 15) == 0) { ui->activePage = PAGE_VISUALS; ui->menuSelection = 0; }
    else if (strncmp(selItem, "ZONE CONFIG", 11) == 0 || strncmp(selItem, "  [BOUNDARIES]", 14) == 0) { ui->activePage = PAGE_ZONES; ui->menuSelection = 0; }
    else if (strncmp(selItem, "TARGET DATA", 11) == 0 || strncmp(selItem, "  [GAIN/FILTER]", 15) == 0) { ui->activePage = PAGE_DATA; ui->menuSelection = 0; }
    else if (strncmp(selItem, "DEV OPTIONS", 11) == 0) { ui->activePage = PAGE_DEV; ui->menuSelection = 0; }
    else if (strncmp(selItem, "USER GUIDE", 10) == 0) { ui->state = STATE_GUIDE; ui->guidePage = 0; }
    else if (strncmp(selItem, "[ Exit Menu ]", 13) == 0) { ui->state = STATE_RADAR_VIEW; }
}
...
inline void VisualsMenuView::handleMenuClick(UIManager* ui) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "<- Back", 7) == 0) { ui->activePage = PAGE_MAIN; ui->menuSelection = 0; }
    else { ui->state = STATE_MENU_EDIT; }
}
inline void VisualsMenuView::executeMenuEdit(UIManager* ui, int dir) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "Theme:", 6) == 0) {
    ...
    if (strncmp(selItem, "Icon:", 5) == 0) {
        int ic = (int)ui->targetIcon + dir;
        if (ic > 3) ic = 0; if (ic < 0) ic = 3;
        ui->targetIcon = (TargetIcon)ic;
        return;
    }
    if (strncmp(selItem, "Text Size:", 10) == 0) { ui->uiTextSize += dir; if (ui->uiTextSize < 1) ui->uiTextSize = 1; if (ui->uiTextSize > 2) ui->uiTextSize = 2; return; }
    if (strncmp(selItem, "UI Scale:", 9) == 0) { ui->uiScale += dir * 0.1f; if (ui->uiScale < 0.5f) ui->uiScale = 0.5f; if (ui->uiScale > 2.0f) ui->uiScale = 2.0f; return; }
    if (strncmp(selItem, "Sweep Line:", 11) == 0) { ui->sweepLineEnabled = !ui->sweepLineEnabled; return; }
    if (strncmp(selItem, "Sweep Mode:", 11) == 0) { ui->simulatedSweep = !ui->simulatedSweep; return; }
    if (strncmp(selItem, "Trails:", 7) == 0) { ui->trailLength += dir; if (ui->trailLength < 0) ui->trailLength = 0; if (ui->trailLength > 10) ui->trailLength = 10; return; }
    if (strncmp(selItem, "Grid:", 5) == 0) { ui->gridEnabled = !ui->gridEnabled; return; }
    if (strncmp(selItem, "Boot Anim:", 10) == 0) { ui->startupAnimEnabled = !ui->startupAnimEnabled; return; }
}
...
inline void ZonesMenuView::executeMenuEdit(UIManager* ui, int dir) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "Warn Zone:", 10) == 0) {
    ...
    if (ui->zoneManager.getWarnPreset() == ZONE_CUSTOM) {
        ...
        if (strncmp(selItem, " W-MinD:", 8) == 0) { z.minDist += dir * 100; if(z.minDist < 0) z.minDist=0; ui->zoneManager.setWarnCustom(z); return; }
        if (strncmp(selItem, " W-MaxD:", 8) == 0) { z.maxDist += dir * 100; if(z.maxDist < z.minDist) z.maxDist=z.minDist; ui->zoneManager.setWarnCustom(z); return; }
        if (strncmp(selItem, " W-MinA:", 8) == 0) { z.minAngle += dir * 5; if(z.minAngle < -90) z.minAngle=-90; ui->zoneManager.setWarnCustom(z); return; }
        if (strncmp(selItem, " W-MaxA:", 8) == 0) { z.maxAngle += dir * 5; if(z.maxAngle > 90) z.maxAngle=90; ui->zoneManager.setWarnCustom(z); return; }
    }
    if (ui->zoneManager.getWarnPreset() != ZONE_OFF) {
        if (strncmp(selItem, "Warn Fuzz:", 10) == 0) {
            int f = ui->zoneManager.getFuzzingThreshold() + dir * 5;
            if (f < 0) f = 0; if (f > 100) f = 100;
            ui->zoneManager.setFuzzingThreshold(f);
            return;
        }
        if (strncmp(selItem, "Warn Time:", 10) == 0) {
            ui->zoneManager.setHistoryWindow(ui->zoneManager.getHistoryWindow() + dir);
            return;
        }
    }
    if (strncmp(selItem, "Dead Zone:", 10) == 0) {
    ...
    if (ui->zoneManager.getDeadPreset() == ZONE_CUSTOM) {
        ...
        if (strncmp(selItem, " D-MinD:", 8) == 0) { z.minDist += dir * 100; if(z.minDist < 0) z.minDist=0; ui->zoneManager.setDeadCustom(z); return; }
        if (strncmp(selItem, " D-MaxD:", 8) == 0) { z.maxDist += dir * 100; if(z.maxDist < z.minDist) z.maxDist=z.minDist; ui->zoneManager.setDeadCustom(z); return; }
        if (strncmp(selItem, " D-MinA:", 8) == 0) { z.minAngle += dir * 5; if(z.minAngle < -90) z.minAngle=-90; ui->zoneManager.setDeadCustom(z); return; }
        if (strncmp(selItem, " D-MaxA:", 8) == 0) { z.maxAngle += dir * 5; if(z.maxAngle > 90) z.maxAngle=90; ui->zoneManager.setDeadCustom(z); return; }
    }
}
...
inline void DataMenuView::handleMenuClick(UIManager* ui) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "<- Back", 7) == 0) { ui->activePage = PAGE_MAIN; ui->menuSelection = 0; }
    else if (strncmp(selItem, "[ Reset Tracking ]", 18) == 0) { ui->actionRequested = 1; ui->state = STATE_RADAR_VIEW; }
    else { ui->state = STATE_MENU_EDIT; }
}
inline void DataMenuView::executeMenuEdit(UIManager* ui, int dir) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "Telemetry:", 10) == 0) {
        int tm = (int)ui->telemetryMode + dir;
        if (tm > 4) tm = 0; if (tm < 0) tm = 4;
        ui->telemetryMode = (TelemetryMode)tm;
        return;
    }
    if (strncmp(selItem, "Sensitivity:", 12) == 0) { ui->sensitivity += dir; if (ui->sensitivity < 1) ui->sensitivity = 10; if (ui->sensitivity > 10) ui->sensitivity = 10; return; }
    if (strncmp(selItem, "Loc Avg:", 8) == 0) { ui->locationAveraging += dir; if (ui->locationAveraging < 1) ui->locationAveraging = 1; if (ui->locationAveraging > 10) ui->locationAveraging = 10; return; }
    if (strncmp(selItem, "Smoothing:", 10) == 0) {
...
inline void DevMenuView::handleMenuClick(UIManager* ui) {
    ...
    else if (strncmp(selItem, "[ RUN SELF TEST ]", 17) == 0 && ui->devRiskAccepted) { ui->actionRequested = 5; ui->state = STATE_SELF_TEST; ui->selfTestDone = false; }
    else if (strncmp(selItem, "[ FACTORY RESET ]", 17) == 0 && ui->devRiskAccepted) { ui->state = STATE_CONFIRM_RESET; }
    else if (strncmp(selItem, "[ EXPORT CONFIG ]", 17) == 0 && ui->devRiskAccepted) { ui->actionRequested = 2; ui->state = STATE_RADAR_VIEW; }
    else if (strncmp(selItem, "[ IMPORT CONFIG ]", 17) == 0 && ui->devRiskAccepted) { ui->state = STATE_IMPORTING; }
    else if (strncmp(selItem, "[ VIEW WIFI PASS ]", 18) == 0 && ui->devRiskAccepted) {
    ...
    else if (strncmp(selItem, "[ REGEN WIFI PASS ]", 19) == 0 && ui->devRiskAccepted) { ui->state = STATE_CONFIRM_WIFI_GEN; }
    else { ui->state = STATE_MENU_EDIT; }
}
inline void DevMenuView::executeMenuEdit(UIManager* ui, int dir) {
    const char* selItem = ui->currentMenuItems[ui->menuSelection];
    if (strncmp(selItem, "Accept Risk?", 12) == 0) { ui->devRiskAccepted = !ui->devRiskAccepted; return; }
    if (ui->devRiskAccepted) {
        if (strncmp(selItem, "Motion Comp:", 12) == 0) { ui->motionCompEnabled = !ui->motionCompEnabled; return; }
        if (strncmp(selItem, "Passthrough:", 12) == 0) { ui->passthroughMode = !ui->passthroughMode; return; }
        if (strncmp(selItem, "Show StdDev:", 12) == 0) { ui->showStdDev = !ui->showStdDev; return; }
        if (strncmp(selItem, "[ FACTORY RESET ]", 17) == 0) {
