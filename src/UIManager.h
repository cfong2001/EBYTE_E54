...
inline void MainMenuView::handleMenuClick(UIManager* ui) {
    // Optimize: Prevent heap fragmentation by replacing Arduino String allocation and startsWith() with zero-allocation C-string comparisons (strncmp).
...
inline void DevMenuView::handleMenuClick(UIManager* ui) {
    ...
    else if (strncmp(selItem, "[ REGEN WIFI PASS ]", strlen("[ REGEN WIFI PASS ]")) == 0 && ui->devRiskAccepted) { ui->state = STATE_CONFIRM_WIFI_GEN; }
    else { ui->state = STATE_MENU_EDIT; }
}
