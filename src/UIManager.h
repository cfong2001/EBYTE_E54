...
public:
    ...
    void drawBootScreen() {
        ...
        const char* bootText = "";
        if (elapsed < 300) bootText = "INIT";
        else if (elapsed < 600) bootText = "CALIBRATING";
        else if (elapsed < 1000) bootText = "SCANNING...";

        if (bootText[0] != '\0') {
            int tw = sprite.textWidth(bootText);
            sprite.setCursor((tft.width() - tw) / 2, 120);
            sprite.print(bootText);
        }
...
inline void DevMenuView::executeMenuEdit(UIManager* ui, int dir) {
    ...
    if (ui->devRiskAccepted) {
        ...
        if (idx++ == ui->menuSelection) {
            ...
            ui->sprite.fillSprite(ui->activeTheme.danger); // activeTheme.danger
            ui->sprite.setTextColor(ui->activeTheme.primary);
