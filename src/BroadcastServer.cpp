...
void BroadcastServer::begin() {
    if (isRunning) return;
        ...
        uint32_t bt_ssp_pin = esp_random() % 1000000;
    const char* bt_devname = "ESP32-Radar-Tracker";
    char pwd[9];
    snprintf(pwd, sizeof(pwd), "rn%06lu", (unsigned long)bt_ssp_pin);
    currentPassword = String(pwd);
    WiFi.softAP(bt_devname, pwd); // Open AP
